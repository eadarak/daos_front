...
export default function Ajouter_Formation_Niveau({ niveau }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (selectedFormations !== null) {
            fetch(`${MAQUETTE_URL}niveau/${niveau.idNiveau}/formations/${selectedFormations.idFormation}`, {
                method: 'PUT',
                headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    throw new Error("Erreur lors de l'ajout du formation à l'Niveau");
                }
                return response.json();
            })
            .then(data => {
                console.log('Formation ajouté avec succès à l\'Niveau:', data);
                setOpen(false);
            })
            .catch(error => {
                console.error("Une erreur s'est produite lors de l'ajout du formation à l'Niveau:", error);
            });

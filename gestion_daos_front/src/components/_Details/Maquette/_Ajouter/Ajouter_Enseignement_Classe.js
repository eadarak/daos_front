...
export default function Ajouter_Enseignement_Classe({ classe }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (selectedEnseignements !== null) {
            fetch(`${MAQUETTE_URL}classe/${classe.idClasse}/enseignements/${selectedEnseignements.idEnseignement}`, {
                method: 'PUT',
                headers: {
            ...
            .then(response => {
                if (!response.ok) {
                    throw new Error("Erreur lors de l'ajout du enseignement à l'Classe");
                }
                return response.json();
            })
            .then(data => {
                console.log('Enseignement ajouté avec succès à l\'Classe:', data);
                setOpen(false);
            })
            .catch(error => {
                console.error("Une erreur s'est produite lors de l'ajout du enseignement à l'Classe:", error);
            });

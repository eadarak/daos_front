...
function Ajouter_Rep_Ens ({ens}) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        console.log(`${REPARTITION_URL}/enseignant/${ens.idEns}/repartition`);
        fetch(`${REPARTITION_URL}/enseignant/${ens.idEns}/repartition`, {
            method: 'POST',
        ...
        .then(data => {
            console.log('Une nouvelle repartion a été ajoutée avec succès', data);
            setFormData(modelRepartition);
            setOpen(false);
            redirectToUeDetails(ens);
        })
    ...
    const  redirectToUeDetails = (ens) => {
       <DetailsEnseignant ens={ens}/>
    }
...
export default Ajouter_Rep_Ens;

...
function Modifier_EmploiDuTemps({ emploi, open, onClose }) {
    const navigate = useNavigate();
    const [data, setData] = React.useState(emploi);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        fetch(`${EMPLOI_URL}/emploi-du-temps/${emploi.idEmploi}`, {
            method: 'PUT',
            ...
            .then(data => {
                console.log('Emploi modifié avec succès', data);
                onClose();
                navigate('/listes-emploi-du-temps', { replace: true });
                window.location.reload();
            })
            .catch(err => {
                console.error("Une erreur s'est produite lors de la modification de l'emploi :", err);
            });
    };
...
export default Modifier_EmploiDuTemps;

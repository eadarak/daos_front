...
function Modifier_Semestre ({ semestre, open, onClose }) {
    ...
    const [data, setData] = React.useState(semestre);
    ...
    const handleSubmit = (e) => {
        ...
        fetch(`${MAQUETTE_URL}semestre/${semestre.idSemestre}`, {
            method : 'PUT',
        ...
        .then(data => {
            console.log('Semestre modifiée avec succès', data);
            setData(modelSemestre);
            onClose();
            navigate('/listeSemestre', { replace: true });
            window.location.reload();
        })
        .catch(err => {
            console.error("Une erreur s'est produite lors de la modification du Semestre :", err);
        });
    }
                    ...
                    <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <TextField
                                    ...
                                    required
                                    fullWidth
                                    value={data.libelleSemestre}
                                    onChange={handleChange}
                                />
                            </Grid>
                            <Grid item xs={6}>
                                <TextField
                                    id="descriptionSemestre"
                                    label="Description Semestre "
                                    fullWidth
                                    required
                                    value={data.descriptionSemestre}
                                    onChange={handleChange}
                                />
                            </Grid>
...
export default Modifier_Semestre;
